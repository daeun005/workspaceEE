...
public class DispatcherServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			/*
			 * <<Properties객체>> -------------------------------------------- |key(String) |
			 * value(String) | -------------------------------------------- |/guest_main.do
			 * |com..GuestMainController| -------------------------------------------
			 * |/guest_list.do |com..GuestListController|
			 * ------------------------------------------- |/guest_view.do
			 * |com..GuestViewController| --------------------------------------------
			 */
			Set commandKeySet = controllerMappingProperties.keySet();
			Iterator<String> commandKeyIterator = commandKeySet.iterator();
			System.out.println("------설정파일[" + configFile + "]을 이용해서 Controller 객체생성-------");
			while (commandKeyIterator.hasNext()) {
				...
				/*
				 * "com.itwill.guest.controller.GuestMainController" Controller클래스이름을 사용해서
				 * Controller객체생성 1. Controller클래스 이름을 사용해서 클래스를 메모리에로딩 2. 메모리에로딩된 클래스의 기본생성자를
				 * 호출해서 객체생성
				 */
		 ...
		 * ----------------------------------------------- |/guest_list.do
		 * |com..GuestListController객체| -----------------------------------------------
		 * |/guest_view.do |com..GuestViewController객체|
		 * ------------------------------------------------
		 */
		/*********************************
		 * 직접생성********************* handlerMapping.put("/guest_main.do", new
		 * GuestMainController()); handlerMapping.put("/guest_list.do", new
		 * GuestListController()); handlerMapping.put("/guest_view.do", new
		 * GuestViewController()); handlerMapping.put("/guest_write_form.do", new
		 * GuestWriteFormController()); handlerMapping.put("/guest_write_action.do", new
		 ...
		 * new GuestRemoveActionController()); handlerMapping.put("/guest_error.do", new
		 * GuestErrorController()); System.out.println(">> init:"+handlerMapping);
		 ****************************************************************/
	}
	...
	private void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * 2-1.DispatcherServlet이 클라이언트요청에따른 업무실행할 Controller객체얻기 << handlerMapping객체로부터
		 * 요청command를 처리할Controller객체얻기 >>
		 */

		Controller controller = handlerMapping.get(command);
		/*
		 * 2-2.DispatcherServlet이 Controller객체의 handleRequest메쏘드 실행
		 * 2-3.DispatcherServlet이 Controller객체의 handleRequest메쏘드 실행반환값인 forwardPath를
		 * 받는다.
		 */
		...
		/*
		 * << pathArray >> |----0---|-----------------1------------| |forward
		 * |/WEB-INF/views/guest_main.jsp | |--------|------------------------------|
		 * 
		 * |----0---|-------1-----| |redirect|guest_main.do| |--------|-------------|
		 */
